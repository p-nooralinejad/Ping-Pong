...
BOOL CreateGLWindowA(char* title, int width, int height, int bits, bool fullscreenflag)
{
	...
	hWnd=CreateWindowExA(	dwExStyle,							// Extended Style For The Window
								"OpenGL",							// Class Name
								"",/*("Ping Pong. Our PAK Group Computer AI Project Using RL for the 4th Expo. of Science & Technology ",*/								// Window Title
								...
								NULL)))								// Dont Pass Anything To WM_CREATE
	{
		...
		MessageBoxA(NULL,"dWindow Creation Error.","ERROR",MB_OK|MB_ICONEXCLAMATION);
		return FALSE;								// Return FALSE
	}
...
//	if (MessageBoxA(NULL,"Would You Like To Run In Fullscreen Mode?", "Start FullScreen?",MB_YESNO|MB_ICONQUESTION)==IDNO)
	...
	if (!CreateGLWindowA("Ping Pong",1100,700,30,fullscreen))
	{
	...
	while(!done)									// Loop That Runs While done=FALSE
	{
		if (PeekMessage(&msg,NULL,0,0,PM_REMOVE))	// Is There A Message Waiting?
		{
			...
			{
				if (t.is_defined){
					ofstream outttt("Data/Files/toop.txt");
					outttt << t;
					outttt.close ( );
				}
		...
		else										// If There Are No Messages
		{
			...
			if (keys[VK_F1])						// Is F1 Being Pressed?
			{
				...
				if (!CreateGLWindowA("NeHe's Color Tutorial",640,480,16,fullscreen))
				{
					return 0;						// Quit If Window Was Not Created
				}
			}
